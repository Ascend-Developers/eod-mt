...
<div class="form-group">
    <label for="">T1<span style="font-size: 70%"> (Arrival to cabinet)</span></label>
    <input type="number" min=0 class="form-control" name="t1" @if(isset($wt)) value="{{$wt->t1}}" @else value="{{ old('t1') }}" @endif>
</div>

<div class="form-group">
    <label for="">T2<span style="font-size: 70%"> (At cabinet)</span></label>
    ...
    <select name="site_id" class="select2 select2" id="large-select-multi">
        ...
        @if(Auth::user()->type == "agent")
            @foreach (Auth::user()->sites as $site)
                <option value="{{$site->_id}}" @if( (isset($wt->site_id)) || (old('site_id') && in_array($site->id, old('site_id')) ) )selected="selected" @endif>
                    {{$site->name}}
                </option>
            @endforeach
        @else
